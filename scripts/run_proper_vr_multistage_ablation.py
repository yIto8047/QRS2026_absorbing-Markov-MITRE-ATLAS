#!/usr/bin/env python3
"""
run_proper_vr_multistage_ablation.py
======================================
few-shotアブレーション用: S2の文脈は元モデル(d3/d4)のものを使い、
S3への応答だけをnofsモデルに問い合わせる。
"""
...
def main():
    ...
    ap.add_argument("--s2_context_config", required=True, choices=["d0", "d1", "d2", "d3", "d4"])
    ap.add_argument("--query_model", required=True)
    ap.add_argument("--label", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--timeout", type=int, default=300)
    args = ap.parse_args()

    s2_provider = f"llama3-{args.s2_context_config}"

    s2_by_code = load_s2_by_provider(s2_provider)
    s3_rows = load_s3_attacks()
    print(f"[setup] S2 context source ({s2_provider}): {len(s2_by_code)} unique access_codes")
    print(f"[setup] query model: {args.query_model}")
    ...
    for i, row in enumerate(usable, 1):
        ...
        t0 = time.time()
        try:
            response = call_ollama(messages, DEFAULT_OPTIONS, args.timeout, args.query_model)
        ...
        if i % 10 == 0 or i == len(usable):
            ...
            avg = total_elapsed / i
            print(f"[{args.label}/proper_ablation] [{i}/{len(usable)}] "
    ...
    total = len(usable)
    print(f"[{args.label}/proper_ablation] DONE. total={total}, leaked={leak_count} "
